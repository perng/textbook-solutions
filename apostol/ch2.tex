...
\noindent\bigskip\noindent\textbf{Solution:}
\begin{enumerate}[label=(\alph*)]
\item If \( x \in f^{-1}(Y_1 \cup Y_2) \), then \( f(x) \in Y_1 \cup Y_2 \Rightarrow x \in f^{-1}(Y_1) \cup f^{-1}(Y_2) \), and vice versa.
...
\bigskip\noindent\textbf{Solution:}
\begin{enumerate}[label=(\alph*)]
\item Each circle is determined by a rational radius and two rational coordinates → set is countable.
\item Each disjoint interval must contain a distinct rational number → inject into \( \mathbb{Q} \), which is countable.
\end{enumerate}\qed
